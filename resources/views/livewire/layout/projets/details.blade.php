...
<div style="width: 90%" class="container   mt-6 ml-40">
    ...
    <div class="p-6 mt-8 bg-white dark:bg-gray-800 sm:rounded-lg shadow-md">
        ...
        <div class="mt-8">
            @if(count($plans) > 0)
                <center><h3 class="text-lg font-semibold mb-2">Plans du projet</h3></center>
                @foreach($plans as $index => $plan)
                    <div class="mb-4">
                        <img src="{{ asset('storage/' . $plan) }}" alt="Plan du projet" class="w-full h-auto mb-4">
                        <button wire:click="deletePlan({{ $index }})"
                                class="text-white bg-red-600 hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center transition-transform transform hover:scale-105">
                            Supprimer Plan
                        </button>
                    </div>
            ...
            <h3 class="text-lg font-semibold mb-2 mt-8">Uploader les plans du projet</h3>
            <form wire:submit.prevent="uploadPlans" enctype="multipart/form-data">
                <input type="file" wire:model="planFiles" multiple class="mb-2">
                @error('planFiles.*') <span class="error text-red-600">{{ $message }}</span> @enderror
                <button type="submit"
                        class="mt-2 bg-green-600 text-white py-2 px-6 rounded-lg shadow-lg hover:bg-green-500 focus:outline-none focus:ring-2 focus:ring-green-600 focus:ring-opacity-50 transition duration-300">
                    Uploader les Plans
                </button>
