<div class="max-w-lg mx-auto p-8 bg-white rounded-lg shadow-lg mt-10">
            <b><i><h3 style="text-align: center">Formulaire de modification de projet</h3></i></b>
<br>
    <form class="space-y-6" wire:submit.prevent="submit">
        <!-- Nom du projet -->
        <div>
            <label for="nomProjet" class="block text-sm font-medium text-gray-900 dark:text-white mb-2">Nom du projet</label>
            <input type="text" name="nomProjet" id="nomProjet" wire:model="nomProjet" value="{{old('nomProjet',$projet->nomProjet)}}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Saisir le nom du projet" required />
        ...
        <div>
            ...
            <select id="chefProjet" wire:model="chefProjet" value="{{old('chefProjet')}}" required class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
        ...
        <div>
            ...
            <input type="number" name="budget" id="budget" wire:model.blur="budget" value="{{old('budget',$projet->budget)}}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Saisir le nom du projet" required />
...
<div>
                ...
                @foreach($ouvriersListes as $ouvrier)
                    <li>
                        <div class="flex items-center p-2 rounded hover:bg-gray-100 dark:hover:bg-gray-600">
                            <input id="checkbox-item-{{ $ouvrier->id }}" type="checkbox" wire:model="ouvriers" value="{{ $ouvrier->id }}" class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:bg-gray-700 dark:border-gray-600" {{ in_array($ouvrier->id, $ouvrierProjets) ? 'checked' : '' }}>
    ...
    @endif
       ...
       <div class="w-full mb-5 group flex flex-row">
            ...
            <div class="w-1/4">
<x-radio id="rounded-lg" wire:model="statut" value="{{ old('statut', $projet->statut ?? 'initial') }}" rounded="lg" label="Initial" value="initial" xl />
            </div>
            <div class="w-1/4">
<x-radio id="rounded-lg" wire:model="statut" value="{{ old('statut', $projet->statut ?? 'en_cours') }}" rounded="lg" label="En cours" value="en_cours" xl />
            </div>
            <div class="w-1/4">
<x-radio id="rounded-lg" wire:model="statut" value="{{ old('statut', $projet->statut ?? 'terminer') }}" rounded="lg" label="Terminé" value="terminer" xl />
            </div>
       ...
       <div>
        <input wire:model="dateDeDebut" value="{{ old('dateDeDebut', $projet->dateDeDebut) }}"  type="date" name="dateDeDebut" id="dateDeDebut" placeholder="Date de debut">
            @if ($errors->get("dateDeDebut"))
                @foreach ($errors->get("dateDeDebut") as $error)
                    <span class="text-red-500 text-sm">{{ $error }}</span>
                @endforeach
            @endif
        <input wire:model="dateDeFin" value="{{ old('dateDeFin', $projet->dateDeFin) }}" type="date" name="dateDeFin" id="dateDeFin" placeholder="Date de fin">
        ...
        <div>
            ...
            <textarea id="description" name="description" wire:model.blur="description" value="{{old('description',$projet->description)}}" rows="4" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Saisir la description du projet" required >{{ old('description', $projet->description) }}</textarea>
        ...
        <div class="flex justify-end">
            <button type="button" wire:click="submit" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800">Enregistrer</button>
        </div>
    </form>
</div>