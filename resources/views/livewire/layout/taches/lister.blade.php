<div class="mt-8">
    ...
    <div class="mb-4 flex justify-between items-center px-4">
        <div class="w-1/3">
            <label for="projectFilter" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Filtrer par projet</label>
            <select wire:model="selectedProject" id="projectFilter" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-teal-500 focus:border-teal-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-teal-500 dark:focus:border-teal-500">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 px-4">
        @foreach ($taches as $tache)
            <div class="bg-white shadow-md rounded-lg overflow-hidden transform transition duration-500 hover:scale-105">
                <img class="w-full h-32 object-cover" src="{{ asset('images/btp.png') }}" alt="Image de la tâche">
                <div class="p-4">
                    <a href="{{ route('details-tache', ['id' => $tache->idTache]) }}">
                        <h3 class="text-lg font-bold mb-2 text-gray-900 hover:text-teal-500">{{ $tache->nomTache }}</h3>
                    </a>
